...
use App\Services\TalentoHumano\CuadroTurnos\EmpleadosUnidadService;
use App\Services\TalentoHumano\CuadroTurnos\AccessControlService;
...
class TurnosTerceroController extends Controller
{
    ...
    public function asignarTercero(Request $request, int $unidadId): JsonResponse
    {
        ...
        if (!$asignado) {
            return response()->json(['message' => 'El tercero ya est+í asignado a esta unidad.'], 422);
        }
    ...
    public function desasignarTercero(Request $request, int $unidadId, int $terceroId): JsonResponse
    {
        ...
        if (!$eliminado) {
            return response()->json(['message' => 'No se encontr+¦ la asignaci+¦n.'], 404);
        }
